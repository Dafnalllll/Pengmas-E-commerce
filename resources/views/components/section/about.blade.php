<section class="py-20 px-4">
    <div class="max-w-6xl mx-auto">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            <!-- About Text -->
            <div class="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 shadow-xl" data-aos="fade-right" data-aos-delay="200">
                <h2 class="text-3xl font-bold text-white mb-6" style="font-family: 'Cormorant Garamond', serif;">
                    Tentang Rasa Umbi
                </h2>
                <p class="text-gray-200 text-lg leading-relaxed mb-6">
                    Rasa Umbi adalah brand yang menghadirkan kekayaan rasa asli Indonesia melalui produk-produk berkualitas tinggi yang terbuat dari umbi-umbian nusantara.
                </p>
                <p class="text-gray-200 text-lg leading-relaxed mb-6">
                    Dengan komitmen terhadap kualitas dan cita rasa autentik, kami mengolah bahan-bahan pilihan menjadi produk yang tidak hanya lezat, tetapi juga bergizi dan menyehatkan.
                </p>
                <div class="flex space-x-4">
                    <div class="text-center">
                        <div class="text-3xl font-bold text-orange-500" style="font-family: 'Cormorant Garamond', serif;">100+</div>
                        <div class="text-gray-300 text-sm">Produk</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold text-orange-500" style="font-family: 'Cormorant Garamond', serif;">5000+</div>
                        <div class="text-gray-300 text-sm">Pelanggan</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold text-orange-500" style="font-family: 'Cormorant Garamond', serif;">50+</div>
                        <div class="text-gray-300 text-sm">Kota</div>
                    </div>
                </div>
            </div>

            <!-- Features Section -->
            <div class="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 shadow-xl" data-aos="fade-left" data-aos-delay="400">
                <h3 class="text-2xl font-bold text-white mb-6" style="font-family: 'Cormorant Garamond', serif;">
                    Mengapa Memilih Kami?
                </h3>
                <div class="space-y-4">
                    <div class="flex items-start space-x-4">
                        <div class="bg-orange-500 p-2 rounded-full flex-shrink-0">
                            <i class="fas fa-leaf text-white"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-semibold mb-1">100% Natural</h4>
                            <p class="text-gray-300 text-sm">Dibuat dari bahan alami tanpa pengawet buatan</p>
                        </div>
                    </div>

                    <div class="flex items-start space-x-4">
                        <div class="bg-orange-500 p-2 rounded-full flex-shrink-0">
                            <i class="fas fa-award text-white"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-semibold mb-1">Kualitas Premium</h4>
                            <p class="text-gray-300 text-sm">Standar kualitas tinggi dengan sertifikasi halal</p>
                        </div>
                    </div>

                    <div class="flex items-start space-x-4">
                        <div class="bg-orange-500 p-2 rounded-full flex-shrink-0">
                            <i class="fas fa-shipping-fast text-white"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-semibold mb-1">Pengiriman Cepat</h4>
                            <p class="text-gray-300 text-sm">Gratis ongkir untuk pembelian minimal Rp 100.000</p>
                        </div>
                    </div>

                    <div class="flex items-start space-x-4">
                        <div class="bg-orange-500 p-2 rounded-full flex-shrink-0">
                            <i class="fas fa-headset text-white"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-semibold mb-1">Layanan 24/7</h4>
                            <p class="text-gray-300 text-sm">Customer service siap membantu kapan saja</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
