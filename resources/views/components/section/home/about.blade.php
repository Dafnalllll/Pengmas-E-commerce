<section class="py-20 px-4 ">
    <div class="max-w-6xl mx-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <!-- About Text -->
            <div data-aos="fade-right" data-aos-delay="200" data-aos-duration="800">
                <div class="bg-white/60 backdrop-blur-md rounded-2xl p-6 sm:p-8 border border-white/30 shadow-xl hover:scale-105 transition-all duration-500 cursor-pointer">
                    <h2 class="text-2xl sm:text-3xl font-bold text-[#A52A2A] mb-6" style="font-family: 'Cormorant Garamond', serif;">
                        Tentang Kampung Syurga
                    </h2>
                    <p class="text-gray-800 text-base sm:text-lg leading-relaxed mb-6">
                        Rasa Umbi adalah brand yang menghadirkan kekayaan rasa asli Indonesia melalui produk-produk berkualitas tinggi yang terbuat dari umbi-umbian nusantara.
                    </p>
                    <p class="text-gray-800 text-base sm:text-lg leading-relaxed mb-6">
                        Dengan komitmen terhadap kualitas dan cita rasa autentik, kami mengolah bahan-bahan pilihan menjadi produk yang tidak hanya lezat, tetapi juga bergizi dan menyehatkan.
                    </p>
                    <div class="flex flex-col xs:flex-row sm:flex-row space-y-4 xs:space-y-0 sm:space-y-0 sm:space-x-4">
                        <div class="text-center">
                            <div class="text-2xl sm:text-3xl font-bold text-[#A52A2A]" style="font-family: 'Cormorant Garamond', serif;">100+</div>
                            <div class="text-gray-700 text-sm">Produk</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl sm:text-3xl font-bold text-[#A52A2A]" style="font-family: 'Cormorant Garamond', serif;">5000+</div>
                            <div class="text-gray-700 text-sm">Pelanggan</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl sm:text-3xl font-bold text-[#A52A2A]" style="font-family: 'Cormorant Garamond', serif;">50+</div>
                            <div class="text-gray-700 text-sm">Kota</div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Features Section -->
            <div data-aos="fade-left" data-aos-delay="400" data-aos-duration="800" class="mt-8 lg:mt-0">
                <div class="bg-white/60 backdrop-blur-md rounded-2xl p-6 sm:p-8 border border-white/30 shadow-xl hover:scale-105 transition-all duration-700 cursor-pointer">
                    <h3 class="text-xl sm:text-2xl font-bold text-[#A52A2A] mb-6" style="font-family: 'Cormorant Garamond', serif;">
                        Mengapa Memilih Kami?
                    </h3>
                    <div class="space-y-4">
                        <div class="flex items-start space-x-4">
                            <div class="bg-[#A52A2A] p-2 rounded-full flex-shrink-0">
                                <i class="fas fa-leaf text-white"></i>
                            </div>
                            <div>
                                <h4 class="text-[#A52A2A] font-semibold mb-1">100% Natural</h4>
                                <p class="text-gray-700 text-sm">Dibuat dari bahan alami tanpa pengawet buatan</p>
                            </div>
                        </div>

                        <div class="flex items-start space-x-4">
                            <div class="bg-[#A52A2A] p-2 rounded-full flex-shrink-0">
                                <i class="fas fa-award text-white"></i>
                            </div>
                            <div>
                                <h4 class="text-[#A52A2A] font-semibold mb-1">Kualitas Premium</h4>
                                <p class="text-gray-700 text-sm">Standar kualitas tinggi dengan sertifikasi halal</p>
                            </div>
                        </div>

                        <div class="flex items-start space-x-4">
                            <div class="bg-[#A52A2A] p-2 rounded-full flex-shrink-0">
                                <i class="fas fa-shipping-fast text-white"></i>
                            </div>
                            <div>
                                <h4 class="text-[#A52A2A] font-semibold mb-1">Pengiriman Cepat</h4>
                                <p class="text-gray-700 text-sm">Gratis ongkir untuk pembelian minimal Rp 100.000</p>
                            </div>
                        </div>

                        <div class="flex items-start space-x-4">
                            <div class="bg-[#A52A2A] p-2 rounded-full flex-shrink-0">
                                <i class="fas fa-headset text-white"></i>
                            </div>
                            <div>
                                <h4 class="text-[#A52A2A] font-semibold mb-1">Layanan 24/7</h4>
                                <p class="text-gray-700 text-sm">Customer service siap membantu kapan saja</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
